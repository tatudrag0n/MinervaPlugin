...
final class SlotMachineManager implements Listener {
   ...
   private void startSpin(Player player, Block shelf, SlotMachineManager.Difficulty difficulty) {
      if (this.activeSessions.containsKey(player.getUniqueId())) {
         ...
         if (this.busyMachines.containsKey(machine)) {
            ...
            player.sendMessage("§e" + difficulty.wager + "MPでスロットを回します。");
   ...
   private void determineResult(SlotMachineManager.SpinSession session, int rerolls) {
      ...
      if (roll < winChance) {
         ...
         this.revealResult(session, List.of(symbol, symbol, symbol), () -> {
            this.handleWin(session, symbol);
            this.finishSession(session);
         });
      } else if (roll < winChance + 0.1 && rerolls < 8) {
         this.revealResult(
            session, List.of(SlotMachineManager.Symbol.REDSTONE, SlotMachineManager.Symbol.REDSTONE, SlotMachineManager.Symbol.REDSTONE), () -> {
         ...
         this.revealResult(session, loss, () -> {
   ...
   private void revealResult(final SlotMachineManager.SpinSession session, final List<SlotMachineManager.Symbol> result, final Runnable onComplete) {
      session.task = (new BukkitRunnable() {
