...
final class WorldEditHook {
    ...
    int[] readSchematicSize(File file) {
        ...
        try (var input = Files.newInputStream(file.toPath())) {
            ...
            Object dimensions = call(clipboard, "getDimensions", new Class<?>[0]);
            return new int[]{
                    coordinate(dimensions, "x", "getBlockX"),
                    coordinate(dimensions, "y", "getBlockY"),
                    coordinate(dimensions, "z", "getBlockZ")
            };
        } catch (Throwable e) {
            plugin.getLogger().warning("WorldEdit schematic size read failed: " + e.getMessage());
            return null;
        }
    }
    ...
    private int coordinate(Object vector, String primaryMethod, String fallbackMethod) throws Exception {
        try {
            return ((Number) call(vector, primaryMethod, new Class<?>[0])).intValue();
        } catch (NoSuchMethodException ignored) {
            return ((Number) call(vector, fallbackMethod, new Class<?>[0])).intValue();
        }
    }
}
